...
CREATE TABLE user_settings (
    ...
    CONSTRAINT ck_settings_auto_sign_limit CHECK (auto_sign_limit >= 0),
...
CREATE TABLE friend_requests (
    ...
    CONSTRAINT ck_friend_requests_not_self CHECK (from_user_id <> to_user_id),
...
CREATE TABLE sport_auto_sign_entries (
    ...
    target_section_name varchar(255) NOT NULL,
    target_section_level bigint NOT NULL,
...
CREATE TABLE sport_free_sign_entries (
    ...
    notification_attempts integer NOT NULL DEFAULT 0,
    max_notification_attempts integer NOT NULL DEFAULT 10,
    CONSTRAINT ck_free_sign_attempts CHECK (notification_attempts >= 0),
    CONSTRAINT ck_free_sign_max_attempts CHECK (max_notification_attempts > 0),
...
CREATE UNIQUE INDEX uq_free_sign_not_cancelled ON sport_free_sign_entries (user_id, lesson_id) WHERE NOT is_cancelled;
...
CREATE TABLE sport_update_logs (
    ...
    received_lessons integer NOT NULL,
    new_lessons_added integer NOT NULL,
    updated_lessons integer NOT NULL,
    skipped_lessons integer NOT NULL,
    error_category varchar(16),
    CONSTRAINT ck_sport_update_duration CHECK (duration_millis >= 0),
    CONSTRAINT ck_sport_update_received CHECK (received_lessons >= 0),
    CONSTRAINT ck_sport_update_new CHECK (new_lessons_added >= 0),
    CONSTRAINT ck_sport_update_updated CHECK (updated_lessons >= 0),
    CONSTRAINT ck_sport_update_skipped CHECK (skipped_lessons >= 0),
    CONSTRAINT ck_sport_update_outcome CHECK (outcome IN ('SUCCESS', 'PARTIAL', 'FAILED')),
    CONSTRAINT ck_sport_update_error_category CHECK (error_category IN ('AUTH', 'NETWORK', 'HTTP', 'MAPPING', 'PERSISTENCE', 'INTERNAL'))
);
...
CREATE TABLE sport_update_logs_new_lessons (
    ...
    CONSTRAINT fk_sport_update_log_link FOREIGN KEY (sport_update_log_id) REFERENCES sport_update_logs (id) ON DELETE CASCADE,
